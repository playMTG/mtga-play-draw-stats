...
| 快速轮抽 BO1 | `QuickDraft_` | 750 宝石 | 7 | 3 | 4 胜（450 宝石 + 1 包；**用户口径**，见下） |
...
来源：优选轮抽与快速轮抽的逐级奖励表、「7 胜／3 负结束」均见 MagicArena Wiki（Premier Draft／
Quick Draft）；选两张轮抽的 4 胜档与「两胜几乎回本」见 Draftsim（Pick-Two Draft，入场 900
宝石）；现开「6 胜可再开一轮」、传统现开「4 胜」见 Draftsim（Sealed）；竞争者轮抽的 0–2 胜
零奖励与「4 胜回本」见 Draftsim（Contender Draft）。
**「回本」一律按宝石 + 补充包（1 包 ≈ 200 宝石）折算，不计开出来的收藏价值**——这个口径
正好复现用户给的两条线（优选 3 胜回了、选二 2 胜回了）。

**快速轮抽的回本线取 4 胜（用户 2026-09-20 拍板），但按上面这个折算口径它其实差一点点**：
4 胜给 450 宝石 + 1 包 ≈ 650，加上第二包 30% 掉率的期望 60，约 710，仍低于入场 750；
严格按算术要到 5 胜（650 + 200 = 850）才过线。取 4 是用户按自己打下来的手感定的「4 胜就当
回了」——**评语口径听用户的，所以这里按 4 写**。改这个数字只动 `RULES` 里 `QuickDraft_`
那一行的第 5 个字段，切分、档位优先级、话术库都不用碰。
