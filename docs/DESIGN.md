...
### 重复度实测（2026-09-15，回答「体系是不是真的变好了」）

改完之后**必须量一遍**，否则「重新规划」只是自我感觉。口径：把全历史 882 天（有评语的
756 天）各算一遍评语，统计「不同评语条数」「平均每条被用几次」「最高频单句」。

| 口径 | 不同评语条数 | 平均每条被用 | 最高频单句 |
|---|---|---|---|
| 关掉新鲜度（近似改造前） | 307 | 2.46 次 | 「3 连后手，不太顺。」**93 次** |
| 打开新鲜度（v2 引擎） | 356 | 2.12 次 | 同上 47 次 |
| **再扩话术库（现状）** | **464** | **1.63 次** | 「15 场连轴转…」17 次 |

**两个结论：**

1. **新鲜度降权只带来约 15% 的改善，不是主力。** 原因是它只能**重排**当天已有的候选，
   变不出新事实——候选池被三个高频类别占着（`hot_wr` 249 天、`play_draw_streak` 307 天、
   `volume` 226 天），稀有类别（`nemesis` 2 天）填不上空缺。
2. **真正的杠杆是「模板数要跟出现频率成正比」**。量出来才发现：最高频的 4 句全部来自
   `streak_other_*`（3–4 连），而这两个库**各只有 1 条模板**——最常触发的档位给的句子最少。
   `hot_wr`（5 条）同理，单句 40+ 次。补到 8–10 条后最高频单句从 **93 次降到 17 次**。

守卫 `test_every_template_bank_has_enough_variants`：**每个话术库至少 3 条**。
**加新类别时先想它一年会出现几次，再决定写几句。**

**剩下的结构性问题（已知、暂不处理）**：开场类别仍有 39% 的天数落在「连续同类别」里，
因为 63% 的日子由那三个高频类别开场——而那三个恰恰是「今天赢了没有／谁先动／打了多久」，
是当天最重要的三件事。要再降只能靠**更多能在普通日子触发的类别**（本轮的 `arc`／`blitz`／
`revenge` 都是这个方向），但每个新类别都要先量触发率，不能为了多样性硬造。

## 限制赛单轮结果的评语（2026-09-20，用户口径「轮抓的评语要优化」）

**问题**：评语体系里没有任何一类事实知道「这是一轮 draft」。用户截图那天（2026-09-20）打了
5 场选两张轮抓（4 胜 1 负）＋2 场争锋，评语只给「今天手是真的热」＋「几乎全是先手」——
最该说的那件事（这轮轮抽卷了）反而没说出来。用户给的档位口径很具体：选两张轮抓 **4 胜是
「卷了」（要给出激励）、3 胜是「差一把卷，可惜了」、2 胜是「回了」**；真人轮抓 **3 胜回了、
7 胜卷了**；现开等其它限制赛「去网上找贴切的评语」。

**做法**：新增 `app/limited_runs.py` —— 把限制赛对局切成「单次 run」并按档位分档；
`daily_highlights` 把它当成一类新事实（`kind="limited_run"`，主题 `limited`，与当天总胜率／
先后手不是同一个话题，所以**可以同时出现在同一条评语里**）；话术在 `daily_copy.py` 的
`limited_*` 六个库；档位权重挂在候选上（`_LIMITED_WEIGHT`），不塞进以 `kind` 为键的 `_WEIGHT`。

### 上限与「回本」线的来源（都核验过，不凭印象）

| 赛事 | 前缀 | 入场 | 胜场上限 | 负场上限 | 回本线 |
|---|---|---|---|---|---|
| 优选轮抽 BO1（真人） | `PremierDraft_` | 1500 宝石 | 7 | 3 | 3 胜（1000 宝石 + 2 包） |
| 快速轮抽 BO1（机器人） | `QuickDraft_` | 750 宝石 | 7 | 3 | 4 胜（450 宝石 + 1 包；**用户口径**，见下） |
| 选两张轮抽 BO1（4 人） | `PickTwoDraft_` | 900 宝石 | 4 | 2 | 2 胜（800 宝石 + 1 包） |
...
来源（2026-09-20 核验）：优选轮抽与快速轮抽的逐级奖励表、「7 胜／3 负结束」见 MagicArena Wiki
（Premier Draft／Quick Draft）；选两张轮抽的 4 胜档与「两胜几乎回本」见 Draftsim
（Pick-Two Draft，入场 900 宝石）；现开「6 胜可再开一轮」、传统现开「4 胜」见 Draftsim
（Sealed）；竞争者轮抽的 0–2 胜零奖励与「4 胜回本」见 Draftsim（Contender Draft）。

**「回本」的口径**：按宝石 + 补充包（1 包 ≈ 200 宝石，即商店价）折算，**不计开出来的收藏
价值**。这个口径正好复现用户给的两条线（优选 3 胜回了、选二 2 胜回了），也与社区说法对得上。
换个口径（把抓到的牌算成价值）会得出「快速轮抽 0 胜也不亏」这类结论，评语就没有落差可说了。

**快速轮抽的回本线是 4 胜，这是用户口径、不是折算结果**（2026-09-20 用户拍板：「快抓定到
4 胜」）。按上面的折算，4 胜给 450 宝石 + 1 包 ≈ 650，加上第二包 30% 掉率的期望 60，约 710，
**仍低于入场 750**；严格按算术要到 5 胜（650 + 200 = 850）才过线。用户是照自己打下来的手感
定的「4 胜就当回了」，评语口径听用户的，所以写 4。**要改只动 `limited_runs.RULES` 里
`QuickDraft_` 那一行的第 5 个字段**——切分、档位优先级、话术库都不受影响。
副作用：快速轮抽的 `even` 档现在覆盖 4–5 胜两档，`low` 只剩 1–3 胜。

### 档位（`limited_runs.BANDS`，顺序即优先级）

`cap` 打满卷了 → `near` 差一把 → `even` 回了 → `low` 没回本 → `bust` 一场没赢。

- **「差一把」排在「回本」前面**：现开的回本线 6 胜同时是上限 7 的前一档，按用户口径
  「差一把卷，可惜了」更值得说。代价是**现开的 `even` 档永远不会触发**（已知取舍）。
  **用户 2026-09-20 再次确认**：「现开六胜肯定是差一把卷了，可惜啊」——是确认，不是改需求。
  真实库核对（现开 36 轮）：6 胜 2 轮都落 `near`（2022-05-03「就差一把：现开 6 胜，卷到一半
  被拦住了。」／2022-11-27「现开 6 胜收工，离卷就差那一把。」），7 胜 5 轮落 `cap`
  （「现开打满 7 胜，奖池今天归你。」），**`even` 0 轮**。
- 竞争者轮抽 0–2 胜是零奖励，**没有单独分档**，落在 `low`／`bust`（本机历史只有 3 场，
  不值得为它多开两个库）。
- 无败卷掉（0 负）另开一库 `limited_cap_perfect`：0 负只可能出现在打满的那一轮。

### 切分规则

- 一轮结束 = **打满胜场上限**或**吃满负场上限**；计数按 `(event_id, my_deck_id)` 分开。
  - 按 deck 分开是必要的：同一天同一赛事可以开两轮，两轮的临时牌组不同。**第一轮没打完
    就开第二轮**时共用计数器会把第一轮的胜场算到第二轮头上（3-3 被写成 5-3，一句假话）——
    这条有专门的回归测试。
  - 反过来，结束**不看 deck_id 变化**：本机有些赛事会让多个 run 共用一个 deck_id，
    所以判据只能是胜/负上限。
- **只认打完的一轮**：半途的 2-1 不进评语——宁可不说，也不能把中途成绩说成一轮结果。
- **不在表里的赛事一律不评**：`MWM_*`／`Decathlon*`／`Omniscience*`／`Cube*` 这类活动赛没有
  固定的 run 边界（本机 `MWM_OmniscienceDraft_20251223` 单个 deck 组 132 场、89 胜 43 负，
  能一直打），套上「7 胜 = 卷」会把活动赛误判成天天卷。
- **跨天的一轮**（昨天开、今天收）算「今天打完的」：`insights.daily_report` 用**全量** `dated`
  切一次（毫秒级，不新增持久化状态），按结束日归日，经 `context["limited_runs"]` 传进
  `highlights()`。只切当天那几行会漏掉 12 轮（全历史 246 轮里的 5%）；漏掉只是不说、不会说错，
  所以没有 context 的调用者（测试、最近 7 天的重算）退回按当天切。
- 一天多轮时**只评最亮的一轮**（先比档位、再比胜场、最后比结束得晚的）。已知简化：
  不报「今天开了 N 轮」——64 天里有多轮，这句以后可以补。

### 实测

- 全历史 887 个有对局的日子、246 轮：**111 天（12.5%）**有打完的一轮，属于稀有档。
  档位分布 `low` 121 ／ `even` 48 ／ `cap` 37 ／ `bust` 23 ／ `near` 17；按赛制
  `quick` 97 ／ `premier` 92 ／ `sealed` 36 ／ `picktwo` 17 ／ `trad_sealed` 3 ／ `trad` 1。
- 「异常 run」（场数超过上限之和）实测 **0** —— 切分规则与真实数据自洽。
- 重复度（口径与 2026-09-15 相同，763 个有评语的日子）：不同评语条数 **464 → 526**、
  平均每条被用 1.63 → **1.45 次**；`limited_run` 成为 **109 天**的开场（占 14%）。
  单句最高频仍是既有的 `hot_wr_perfect`（25 次），新库最高 6 次。
- 性能：`/api/daily` 全流程 **282ms**（基线 360ms 里的 334ms 是 `records()`），切分无可测开销。

### 修掉的一个真 BUG（量出来才发现的）

`best_run` 的 `rank` 第一版直接返回 `BANDS.index`（档位越靠前越好），而 `max` 取的是**最大**
元组——于是它挑中的是「没回本」那一轮。真实表现：2022-01-16 明明有一轮快速轮抽 7-1 卷了，
评语却写「3 胜，奖励没够本」；2026-08-16 有 4-1 卷了却写「1 胜收工」。改成返回
`-BANDS.index` 后修正。**「评语里的胜场必须与它引用的那一轮一致」这条测试就是为此加的**
——胜场写错比不说更糟。

### 测试与守卫

- `tests/test_limited_runs.py` 15 条：档位与用户口径一致（选二 4/3/2、优选 7/3）、活动赛不评、
  打满／吃满即结束、半途不报、两轮分开计数、`best_run` 取最亮的、评语里的胜场与引用的一轮
  一致、与「今天手热」共存、跨天一轮走 context、**每个档位都要有话术库**（漏库会静默吐空句）。
- 空转验证：把 `rank` 改回正下标 → `test_best_run_prefers_the_better_band` 失败；把切分键
  去掉 `my_deck_id` → `test_same_event_twice_in_a_day_needs_different_decks` 失败。均已还原。
- 顺手删掉 `tests/test_daily_highlights.py` 里**重复定义两遍**的
  `test_every_template_bank_has_enough_variants`（沙箱把写命令执行两次留下的：同名函数后一份
  覆盖前一份，看着有守卫、其实只有一份在跑）。
- 前端无需改动：评语是一段 `plain` 文本，色条级别复用既有的
  `legendary`／`rare`（`#daily-plain.daily-highlight.*`），所以**没有 `app.js` 改动、
  也就没有缓存失效参数要动**。

## 顶部统计卡与战报的时间范围（2026-09-20，用户口径「这里也像底下一样…」）

**用户原话**：「这里也像底下一样，按今天、本周、本月、总对局来分类筛选。下面的除了今天、
昨天外，加上本周的筛选。」截图指的是**顶部那三张统计卡**（全史总胜率／先手／后手）和
**「每日战报与对局明细」那排日期按钮**。

**两处各自独立，不联动**——这是本轮唯一一个需要拍板的取舍，选独立的理由：
① 用户把两组按钮**分别列出来**（上面要 今天/本周/本月/总对局，下面要 今天/昨天/本周）；
  如果联动，上面点「本月」下面就会多出一个月报、下面点「昨天」上面那三张卡也会变成昨天，
   这两件事他都没要；
② 这个页面本来就有**多个各自独立的时间窗**（套牌详情弹窗的「今天/昨天/近 20 场/近 7 天/
   全部」、近期波动卡的「近 7 天/近 30 天/全部」），再加一个独立范围不破坏既有心智；
③ 顶部那三张卡的身份是「一段时间内的战绩快照」，底部是「某个时间段的战报与明细」，
   两者回答的是不同问题。

代价（已知、接受）：两个范围可以同时停在不同的时间段上。UI 上不会看串——统计卡标题带范围
（「本周总胜率」）、分布条标题旁也有范围（`#pd-dist-scope`）。

### 状态与参数

| 位置 | 状态变量 | 取值 | 请求 |
|---|---|---|---|
| 顶部三张卡 + 分布条 | `statsRange` | `all`（默认）／`today`／`week`／`month` | `/api/overview?range=` |
| 战报 + 对局明细 | `matchRange` | `""`（全部日期，默认**不是**它）／`"week"`／`YYYY-MM-DD` | `/api/daily`、`/api/matches`：单日走 `day=`，本周走 `range=week` |

后端一个函数统一解析范围：`stats.range_bounds(value, today=None) -> (起, 止) | None` +
`stats.range_cond(...)`。**本周从周一起算**（`ref - timedelta(days=ref.weekday())`），
与 `trend_weekly` 的分组口径一致，不是「最近 7 天」；月末的 `month` 是「本月 1 号到今天」。
非法取值抛 `ValueError`，接口层转 **422**（与既有的日期校验同一处理，**不静默退回「全部」**
——那会让页面显示一段不想要的数据却毫无提示）。

**Python 层的参数叫 `range_key`，HTTP 层的参数叫 `range`。** 这不是命名洁癖：`match_list`
里有一个 `for start in range(0, len(match_ids), 500)`，参数一旦叫 `range` 就把它盖掉，
实测报 `TypeError: 'NoneType' object is not callable`——而且是**所有**调 `match_list` 的
测试一起红，报错点看着跟改动毫无关系。`insights.daily_report` 同理（新鲜度降权那段要用
`range(1, 8)`）。

### 「今日…」那三行恒为今天，且不再由战报填

三张卡下面那行「今日 X 胜 Y 负 · 胜率 Z%」原来是 `loadDaily` 用**当天战报**的 `summary`
填的。于是底部一旦切到「昨天」，那行会拿**昨天**的数字自称「今日」——一直如此，没人注意。
现在 `/api/overview` 一次返回两块：所选范围那一块（顶层 `total`／`on_play`／`on_draw`／
`play_draw_rates`）和 `today` 那一块（**恒为今天，同筛选条件，与范围无关**），前端用
`renderTopCards(o)` 一次画完。范围本来就是「今天」时那行是纯复读，清空（`renderTodayLines`
里判 `statsRange !== "today"`）。

两块都从已经加载的行里算（`_totals()` 只多跑两条聚合 SQL），所以范围切到「本周」时
上面一行是本周、下面一行仍是今天，而**不必为「今天」多发一次请求**。

### 周报不跑 `highlights`

单日那套「开场 → 转折」选材引擎是按天写的：`daily_copy.TEMPLATES` 里全是「今天手是真的热」
这类句子，套到一周上会说错话。所以范围报告的 `plain` 换成 `insights._range_plain()` 的一句
汇总，`highlights` 返回空列表：

> 本周 7 天有对局，共 77 场 · 47 胜 30 负 · 胜率 61.0%；先手胜率 67.6%（37 场），后手胜率 50.0%（36 场）。

比较基线也随之下移：单日是「所选日期之前 30 个自然日（不含当天）」，范围是「所选范围之前
30 个自然日（不含该范围）」——`history_summary` 的口径要求基线与所选区间**不重叠**。

响应里回一个 `range` 字段（单日为 `null`）。前端据此**不回写日期控件**：范围报告的 `date`
是范围的**最后一天**，拿它回写会让状态从「本周」悄悄变成某一天。这条有专门的断言。

### 实测（本机全量库，2026-09-20）

| 范围 | 场次 | 胜率 | 先手 | 后手 | 先手胜率 | 后手胜率 |
|---|---|---|---|---|---|---|
| 总对局 | 10837 | 56.9% | 5310 | 5512 | 62.8% | 51.3% |
| 本月（9-01 起） | 233 | 57.9% | 111 | 107 | 67.6% | 47.7% |
| 本周（9-14~9-20） | 77 | 61.0% | 37 | 36 | 67.6% | 50.0% |
| 今天（9-20） | 7 | 85.7% | 5 | 2 | 100% | 50% |

`range=week` 与手工 SQL（`date(start_time/1000,'unixepoch','localtime') BETWEEN '2026-09-14'
AND '2026-09-20'`）逐项一致（47 胜 / 77 场）；`/api/matches?range=week` 的 77 场也只落在
9-14~9-20 这七天。`/api/overview` 加了 `today` 块之后耗时 54ms → 仍在 60ms 量级
（15 秒一次的自动刷新无感）。

### 测试与守卫

- `tests/test_time_ranges.py` 11 条：周一起算（周内 7 天逐天验，周日/周一的边界单独验）、
  关键词与具体日期解析、SQL 形态（单日 `=`、范围 `BETWEEN`）、非法值抛 `ValueError`、
  `overview` 与手工 SQL 对账、**`today` 块恒为今天**、`match_list` 只出本周的行（`day` 与
  `range` 求交）、周报是汇总句且 `highlights` 为空、本周只有一天也出汇总、单日行为不变、
  空范围文案、接口层 422。
- `tests/test_top_stats_ui.cjs`（已登记进 `CJS_TESTS`）：范围切换后标题／数字／注释／分布条
  标注同步，「今日…」行在范围=本周时**仍是今天的数字**、范围=今天时为空、今天没对局时为空、
  切回总对局能重新长出来，以及**渲染统计卡不碰战报与明细的元素**。
- `tests/test_daily_ui.cjs` 更新：状态变量改名 `matchRange`、单日断言 `day=`、新增「本周走
  `range=week` 且不回写日期控件」、顶部「今日…」行改由统计卡那边负责（用哨兵值守住
  「战报不再改写它」）。
- 空转验证（四处，全部还原）：把「今日…」行改成读范围块 → 统计卡 cjs 红；范围报告也回写
  日期控件 → 战报 cjs 红；本周改成「最近 7 天」→ 周一起算那条红；`today` 块改成跟着范围走
  → `test_overview_today_block_is_today_not_the_range` 红。
- 浏览器实测（Chromium 快照 + playwright-core，29 项断言全过、控制台 0 错误）：初始「全史总
  胜率 / 56.9% / 今日 6 胜 1 负 · 胜率 85.7%」；点顶部「本周」→「本周总胜率 61%」而「今日…」
  行**没变**、分布条标注变「本周」、下面的战报与明细**一动不动**；点「今天」→ 标题「今天总胜率
  85.7%」、今日行文本为空且高度 0；底部点「本周」→ `d-week` 高亮、日期输入清空、
  `daily-asof`「本周至今的已记录对局」、战报「本周 7 天有对局…」、明细「共 77 场」、顶部仍是
  「全史总胜率」；底部「全部日期」→ 战报区 computed `display=none`；底部「昨天」→ 日期控件回写
  `2026-09-19`。

## 已知遗留（2026-09-15 体检记下的，等用户拍板）
...
3. **明细表每行的「打标」控件仍是 `<details>`**（全部日期下 459 个）。它是 R10.2 的多标签
   输入控件，不是「点开看数据」的折叠，所以当初在「页面一个折叠都不留」的口径里给它开了例外。
   改成 `<select>` 会动多标签语义（原生 select 做多选在表格单元格里不可用），需要用户定。

### 性能实测（同次体检）

| 接口 | 耗时 |
|---|---|
| `/api/status` · `/api/repeat_opponents` | 50ms · 30ms |
| `/api/overview` · `/api/matches?limit=200` | 70ms · 85ms |
| `/api/daily` · `/api/commanders` · `/api/targeting` | **360ms · 360ms · 290ms** |

拆开 `/api/daily` 的 368ms：**`records()` 全量加载 334ms（91%）**、本轮新增的
`history_context()` 25ms + 最近 7 天重算 1ms（合计 7%）、`highlights` 本身约 0ms。
...
## 套牌名丢失：回填与监听器是两个 builder（2026-09-18 修的真 BUG）

**现象**：用户报 09-18 19:19 那场（对手 Zabel、赛事 `Brawl_Ladder`）在页面上显示「套牌未记录」，
而同一天后三场都正常。

**排查**：
1. 库里那行 `my_deck_tag / my_deck_id / my_deck_version` 全 NULL，后三场都是 `竞技哪失0915`
   / `290a2037-…`。
2. `data/panel.log`：**面板在 19:19:16 重启**；日志里 **19:19:05 进赛事 → 19:19:08 `EventSetDeckV3`
   ＋ `CreateMatch`(带 `CourseDeckSummary`) → 19:19:26 开赛**。重启正好夹在选牌与开赛之间。
3. 复现（真实日志回放）：**从选牌事件之前**回放 → `deck_tag=竞技哪失0915`；
   **从之后**回放 → `deck_tag=None`。完全对上现象。

**根因**：`_course_decks`（赛事→套牌）只活在 `SessionBuilder` 实例里，而**启动回填与接手的监听器
是两个独立 builder**（`_watch_loop` 与 `_boot_tasks` 还是两个并发线程）。MTGA 在**开赛前**才发
选牌事件；监听器一旦从那条之后的水位线起步，就永远学不到映射。后三场没事，是因为 MTGA
每场开赛前都会再发一次。

**与上一个 BUG 同形**（见「对局结果丢失」一节）：**都是「回填吃掉的 builder 状态，监听器看不见」**。
这类问题的通用判据是：**凡是有状态只存在 builder 实例里、而重启会把 builder 换掉，就必须把状态
交棒**（或落库）。目前已知的两处：`_pending_fmr`（→ 已改为按 id 兜底补写）与 `_course_decks`
（→ 本次交棒）。

**修法**：
- `SessionBuilder.course_decks`（只读副本）与 `set_course_decks(mapping)`：补挂映射，
  并且**把已经建好、还没落库的对局也补上**（`_cur` / `_last`）——真实时序里回填要跑十几秒，
  补挂时监听器往往已经建好那场对局了。用 `setdefault` 合并，**不覆盖监听器自己学到的**。
- `backfill()` 回传 `course_decks`（跨文件合并）。
- `main._boot_tasks` 在回填成功后，把映射补挂到 `_builders` 上（模块级引用，
  `_watch_loop_inner` 登记）；`/api/status` 的 `boot_course_decks` 可看到学到的条数。

**自愈**：`backfill` 对仍在增长的 `Player.log` 会整份重读，所以**重启面板同样能修回这一行的
套牌名**。

**测试**：`tests/test_course_deck_handoff.py` 四条（交棒生效、补挂时对局已在建、不覆盖已知套牌、
无映射时不崩）。空转验证：把 `set_course_decks` 改成空操作 → 两条用例报
`assert None == 'Example'` 与「已经建好的对局没被补上套牌名」失败。
**端到端**：用真实日志验证 `{}` → 补挂后 `deck_tag=竞技哪失0915`。
