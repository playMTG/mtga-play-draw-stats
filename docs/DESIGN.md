...
所有统计由后端计算，赛事大类、具体赛事、套牌、Bot 筛选统一传递。日报按服务所在电脑的本地自然日；近 N 天包含今天共 N 个自然日。全史包括未知日期，近期窗口不包括未知日期。
...
尚未完成：全看板 BO 模式筛选及逐局分析入口、构筑对手类型采集/分类流程、跨不同赛事名称的限赛规则规范化、每日变化的可靠原因归因。当前差值是描述性比较，不是“下降由某模式导致”的因果结论。
