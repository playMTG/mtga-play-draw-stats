...
1. **选材引擎**（戏剧分 + 新鲜度降权）——不依赖新数据，先把 `_rank` 换掉。**本次已做**。
2. **三拍结构**（lede／turn／kicker + 主题去重）——改 `highlights()` 的装配与 `plain` 拼装。**本次已做**。
3. 新类别按「数据可得性」逐个加：
   - **本次已做**：崩盘／回魂、闪电局（只需当天）。
   - **下一步**：里程碑、复仇、苦主、稀有对手（要全库上下文，需把 `highlights()` 的入参从「当天行」扩成「当天行 + 历史上下文」）。
