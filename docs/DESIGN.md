...
**口径**：**页面只保留「展开看数据」的折叠**（赛事中文／原文对照表、评语依据、分赛事历史比较、资料覆盖、近期波动明细、构筑版本变更、逐次留牌分布、主将名详情、单场明细、类型打标）。纯说明文字一律写进本文档。据此删掉三个入口：「卡名显示与译名纠正」「主将类型来源」以及嵌套在赛事对照里的「如何纠正本地译名」。`data/card_names.zh.json`、`data/event_names.zh.json` 的用法见 R4／R3 两节。
