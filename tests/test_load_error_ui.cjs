...
const context = vm.createContext({
  $, esc: String,
  loadOverview: stub('总览'), loadMulligans: stub('调度'), loadCommanders: stub('对手主将'),
  loadOpponentTypes: stub('对手类型'), loadMatches: stub('对局明细'), loadStatus: stub('运行状态'),
  loadRankCurve: stub('段位曲线'), loadTargeting: stub('被针对指数'), loadDaily: stub('每日战报'),
});
// 只取 RELOAD_SECTIONS + reload + reportLoadFailures 这一段；九个 loader 用桩注入
vm.runInContext(
...
(async () => {
  ...
  assert.equal(calls.length, 9, '九个区块都应被调用');
  ...
  await context.reload();
  assert.equal(calls.length, 9, '一个区块失败不能阻止其余区块加载');
